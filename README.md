...
[Read README in English](README_en.md)
...
最大の特徴は、Webカメラを用いた**ジェスチャー認識技術**です。従来の全体的なジェスチャー認識に加え、**指先の位置追跡** 技術を導入しました。ユーザーは画面に触れることなく、指でボタンを押すという直感的な操作で、「お振込み」「お引出し」「口座作成」などの取引をスムーズに行えます。

誤操作防止機能として、**同一方向への連続入力防止**や、**画面上部への誤検知フィルタ**を搭載しており、意図しない操作を極限まで排除しています。

### 技術要素

AIモデルには、物体検出・骨格推定のデファクトスタンダードである **Ultralytics YOLOv8-Pose** を採用しました。これにより、手の形状だけでなく「手首」や「肘」の座標を高精度に検出し、そこから指先位置をベクトル演算で推論することで、高速かつ正確なポインティングを実現しています。Python 3.13 環境に対応し、非同期推論によりUIの描画遅延を防いでいます。
...
ボタンのフィードバックには「**押し感**」を実装し、ジェスチャーで選択したボタンが物理的に押し込まれたかのような視覚効果（影の移動とオフセット）をデジタル上で再現しています。ユーザーのアクションに対して、視覚と聴覚の両方でフィードバックを行うことで、より直感的で気持ちの良い操作感を実現しました。
...
`resources/config/atm_config.yml` の `vision` セクションで `min_detection_confidence`（信頼度閾値）を上げることで、誤検出を減らすことが可能です。
</details>
...
A: 本システムは、手首の位置によってどこのボタンを押しているか判定しています。そのため、画面から **75cm** 以上離れて、背筋を伸ばして手首を見せることを意識してご利用ください。
</details>

<details>
<summary>
<b>
Q: pip install -r requirements.txt でインストールできない。エラーが出る。
</b>
</summary>

A: 本システムでは音声再生のために **Pygame** を使用しています。そのため、 **Python 3.14** 以降は対応しておりません。
[こちら](https://www.python.org/downloads/) から **Python 3.13** 以下のバージョンをダウンロードしてインストールしてください。
</details>
