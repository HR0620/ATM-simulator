...
class AccountManager:
    ...
    def transfer(self, source_account, target_account, amount):
        """
        振込処理（sourceから引き出してtargetへ入れる）
        ※今回は「現金振込」か「口座間送金」か要件が曖昧だが、
          左ボタン仕様「振込先入力→金額入力」なので、
          「現金振込（手持ちの現金を相手に入れる）」と解釈もできるが、
          ATMシミュレーターとしては「自分の口座から」が自然。
          しかし、認証フローが明記されていないため、
          今回はシンプルに「振込先指定 -> 入金処理（Deposit）」として実装するか、
          あるいは「現金振込」として扱う。
